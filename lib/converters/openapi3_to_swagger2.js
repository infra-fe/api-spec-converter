...
Converter.prototype.convertOperationParameters = function (operation) {
  var content, param, contentKey, mediaRanges, mediaTypes, example, examples;
  ...
  if (operation.requestBody) {
    ...
    if (content && Object.keys(content).length) {
      ...
      if (contentKey === SUPPORTED_MIME_TYPES.APPLICATION_X_WWW_URLENCODED
        ...
        example = content[contentKey].example;
        examples = content[contentKey].examples;
        ...
        example = content[mediaRanges[0]].example;
        examples = content[mediaRanges[0]].examples;
        operation.parameters.push(param);
      }

      /* 
       * example与examples不可共存。处理逻辑：examples > example > schema.example
       */
      if (!_.isEmpty(examples)) {
        param.examples = Object.entries(examples).reduce((result, [name, data]) => {
...
Converter.prototype.prehandleParameters = function (params) {
  const newParams = []

  for (let param of params) {
    param = this.resolveReference(this.spec, param, false);
    let schema = this.resolveReference(this.spec, param.schema, true)
    if (param.in !== 'body' && schema && schema.type === 'object' && !_.isEmpty(schema.properties)) {
      const objParams = Object.entries(schema.properties).map(([name, data]) => {
        const newObj = _.assign({}, _.omit(param, ['schema']), _.omit(data, ['$ref']))
        newObj.name = name
        return newObj
      })
      newParams.push(...objParams)
    } else {
...
Converter.prototype.convertParameters = function (obj) {
  ...
  obj.parameters = this.prehandleParameters(obj.parameters || []);
  ...
  obj.parameters.forEach((param, i) => {
    ...
    if (param.in !== 'body') {
      ...
      this.convertSchemaSpecialProperties(param, SPECIAL_PROPERTIES);
  ...
  props.forEach(function (prop) {
    ...
    switch (prop) {
      case 'additionalProperties':
        ...
        break;
      case 'type':
        if (value === 'object' && !_.isEmpty(schema.properties)) {
          let firstProperty = Object.entries(schema.properties)[0]
          obj['name'] = firstProperty[0]
          obj['type'] = firstProperty[1].type
          return;
        }
        break;
    }
  ...
  for (code in operation.responses) {
    ...
    if (response.content) {
      ...
      for (mediaRange in response.content) {
        ...
        /*
         * contentV3={schema,example,examples}
         * example与examples不可共存。处理逻辑：examples > example > schema.example
         */
        if (!_.isEmpty(content.examples)) {
          response.examples = Object.entries(content.examples).reduce((result, [name, data]) => {
            result[name] = data.value
            return result
          }, {})
